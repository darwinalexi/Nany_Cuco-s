...
import { Show_product } from "../Modals/Show_product"
...
export const Product_not_avalible=()=>{
    ...
    const [searchName, setSearchName]= useState("");
     ...
     const [show_modal, setmodalsp]=useState(false)




     const show_product=(datos)=>{
      setdatos(datos)
      setmodalsp(true);
    }
    ...
    },        
{
        ...
        <div className="flex space-x-2">
          {type_user === "Propietario(a)" && (
            <>
              ...
              <button onClick={() => show_product(row)}>
                <FontAwesomeIcon icon={faEye} color="#dc2e63" className="size-5" />
              </button>
            </>
          )}
          {type_user === "Administrador(a)" && (
            <>
              <button onClick={() => show_product(row)}>
                <FontAwesomeIcon icon={faEye} color="blue" className="size-5" />
              </button>
            </>
          )}
        </div>
  ...
  {filterdata.length > 0 ? (
    ...
    <p className="flex justify-center">No Hay Registros de ventas</p>
        ...
        {show_modal &&(<Show_product onclose={close_product} data={datos}/>)}          
        </div>
        </>
    )
}