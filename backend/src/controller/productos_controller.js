...
export const register_product =  async(req, res)=>{
    try {
        ...
        const [register] = await pool.query(
            "INSERT INTO productos (nombre, referencia, precio_por_mayor, observaciones, id_usuario, estado,fecha_ingreso, precio_venta,cantidad_vendida,unidades_separadas, cantidad_almacenada, imagen)VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            [nombre, referencia,precio_por_mayor,observaciones,id_usuario,estado, fechaFormateada, precio_venta,cantidad_vendida,cantidad_separada,cantidad_almacenada,imagen]
        )
...
export const update_products = async (req, res) => {
    try {
        ...
        const {
            ...
            precio_por_mayor = productoActual.precio_por_mayor,
            observaciones = productoActual.observaciones,
            id_usuario = productoActual.id_usuario,
            estado = productoActual.estado,
            fecha_ingreso = productoActual.fecha_ingreso,
            precio_venta = productoActual.precio_venta,
            cantidad_vendida = productoActual.cantidad_vendida,
            cantidad_separada = productoActual.unidades_separadas,
            cantidad_almacenada = productoActual.cantidad_almacenada
        } = req.body;

        const fechaFormateada = fecha_ingreso
            ? new Date(fecha_ingreso).toISOString().split('T')[0]
            : productoActual.fecha_ingreso;
        ...
        const [consulta] = await pool.query(
            `UPDATE productos 
             SET nombre=?, referencia=?, precio_por_mayor=?, observaciones=?, id_usuario=?, estado=?, fecha_ingreso=?, 
                 precio_venta=?, cantidad_vendida=?, unidades_separadas=?, cantidad_almacenada=?, imagen=?
             WHERE id=?`,
            [
                nombre, referencia, precio_por_mayor, observaciones, id_usuario, estado,
                fechaFormateada, precio_venta, cantidad_vendida, cantidad_separada,
                cantidad_almacenada, imagen, id
            ]
        );

        if (consulta.affectedRows > 0) {
            res.status(200).json({ mensaje: "El producto ha sido actualizado" });
        } else {
            res.status(404).json({ mensaje: "No se pudo actualizar el producto, es posible que el ID no exista" });
        }
