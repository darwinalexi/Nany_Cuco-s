...
export const create_orders= async(req, res)=>{
    try {
        ...
        const [create]= await pool.query("insert into pedidos (nombre_producto, referencia, observaciones, celular, nombre_persona, estado, usuario, cantidad, fecha_pedido)values(?,?,?,?,?,?,?,?,?)",[nombre_producto,referencia,observaciones,celular,persona,estado,usuario, cantidad,fechaFormateada])
...
try {
    const [show]= await pool.query("select*from pedidos");
    if (show.length>0) {
...
export const sold_value = async (req, res) => {
    try {
        ...
        if (show.length > 0 && listar[0].total_ventas !== null) {
...
export const    count_orders = async (req, res) => {
    try {
        const [result] = await pool.query(" SELECT COUNT(*) as total FROM pedidos where estado='Por Entregar' ");
        if (result[0].total === 0) {
...
export const count_orders_sould  = async (req, res) => {
    try {
